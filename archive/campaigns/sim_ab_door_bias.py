...
COMMON = {"G1_META2": "2", "G1_M2_CFG": "config_meta2_g1payload.json",
          "G1_SIM_ID": "lab_v1_bias", "G1_SIM_NOISE": "1", "G1_M2_L3": "1", "G1_M2_L4": "1",
...
# PREVENCION vs nada: sesgo de aproximacion (Door_BiasPlus, lat_bias +0.12 = hacia la
# DERECHA del robot en B->A) contra la variante recta. Puertas del servo lateral en sus
# valores ORIGINALES en ambos brazos: aqui se mide el sesgo SOLO, no mezclado con el otro
# cambio. Variante forzada por operador para que no la elija el aprendizaje.
ARMS = {
    "recta": {"G1_DOOR_VARIANT": "Door_Direct",
              "G1_SIM_PERC": "1", "G1_PERC": "127.0.0.1:8010", "G1_DOOR_VIS": "1",
              "G1_M2_STATE": "meta2_state_bias_recta.json"},
    "sesgo": {"G1_DOOR_VARIANT": "Door_BiasPlus",
              "G1_SIM_PERC": "1", "G1_PERC": "127.0.0.1:8010", "G1_DOOR_VIS": "1",
              "G1_M2_STATE": "meta2_state_bias_sesgo.json"},
}
ORDER = ["recta", "sesgo", "sesgo", "recta", "recta", "sesgo", "sesgo", "recta",
         "recta", "sesgo", "sesgo", "recta"]
...
for i, arm in enumerate(ORDER):
    ...
    logpath = os.path.join(SCRATCH, "bias_%02d_%s.log" % (i, arm))
    before = newest_run()
...
json.dump(results, open(os.path.join(SCRATCH, "bias_results.json"), "w"), indent=1)
