...
STOP_CLEAR = 0.85     # pared del LASER directamente delante a < esto -> reroute (margen para no comersela)
            ...
            if midas is not None:
                try:
                    ...
                    if ratio > DEPTH_TH:                       # superficie vertical cerca delante
                        ...
                        if ratio > DEPTH_TH + 0.12:       # superficie vertical CERCA (pared blanca/armario que el LiDAR
                            close = True; strong = True   # no ve) -> FUERTE: la cama/control para+rodea (respaldo del laser)
                except Exception:
                    pass
...
def clear_ahead(cdp, off_deg=0.0, mh=None):
    ...
    mh=3 (no 4): pilla paredes BLANCAS/lisas que reflejan poco el LiDAR (pocos impactos) y antes no veia
    -> c0=— casi siempre y se las comia. El anillo fantasma (campo cercano) ya lo filtra fmin=NEAR_BLIND."""
    if mh is None:
        mh = 3
    try:
...
PERSIST_MIN = 3       # valor min en la rejilla para FIJAR celda en el mapa (3 no 4: pilla paredes blancas que reflejan poco
...
ROBOT_R = 0.22        # radio del robot (m) para la holgura del DWA (deja pasar huecos ~0.45m)
DWA_SPEED = {0.40: 0.27, 0.28: 0.19, 0.0: 0.0}   # ly -> m/s (medido)


def dwa_step(x, y, yaw, carrot, obs_pts):
    """Planificador local DWA (la logica del firmware): simula varias motrices candidatas ~1.4s, las
    PUNTUA por (progreso hacia el carrot del path A*) + (holgura a obstaculos) - (desvio), descarta las que
    colisionan, y devuelve la mejor (lx,ly,rx,ry,label). Suave, sin saltar entre modos reactivos -> no se
    enrosca. obs_pts = puntos de obstaculo (m) cercanos (paredes LiDAR + camara)."""
    yawr = math.radians(yaw); cgx, cgy = carrot
    d0 = math.hypot(cgx - x, cgy - y)
    cands = [(0.40, 0.0), (0.40, -AV_TURN), (0.40, AV_TURN), (0.28, -AV_TURN), (0.28, AV_TURN),
             (0.0, -AV_TURN), (0.0, AV_TURN), (0.0, 0.0)]
    best = None; bs = -1e18; blabel = "STOP "
    for ly, rx in cands:
        sp = DWA_SPEED[ly]; wz = -1.8 * rx           # rx+ -> yaw BAJA; |0.45|-> ~0.81 rad/s
        px, py, pa = x, y, yawr; minclr = 9.9; ok = True
        for _ in range(11):                           # ~2.2s, dt=0.2s (mira mas adelante -> reacciona antes)
            pa += wz * 0.2; px += sp * math.cos(pa) * 0.2; py += sp * math.sin(pa) * 0.2
            if obs_pts:
                cl = min((px - ox) ** 2 + (py - oy) ** 2 for ox, oy in obs_pts) ** 0.5
                if cl < minclr:
                    minclr = cl
                if cl < ROBOT_R:                      # esta trayectoria CHOCA -> descartada
                    ok = False; break
        if not ok:
            continue
        de = math.hypot(cgx - px, cgy - py)
        prog = d0 - de                                # cuanto se acerca al carrot
        be = abs((math.degrees(math.atan2(cgy - py, cgx - px)) - math.degrees(pa) + 180) % 360 - 180)
        score = 3.0 * prog + 1.0 * min(minclr, 1.2) - 0.005 * be + (0.25 if sp >= 0.27 else 0.0)
        if score > bs:
            bs = score; best = (ly, rx); blabel = ("DWA-F" if sp > 0 else "DWA-T")
    if best is None:                                  # todo colisiona -> parar (el desatasco lo sacara)
        return (0, 0, 0, 0, "STOP ")
    return (0, best[0], best[1], 0, blabel)
    ...
    try:
        while time.time() - t0 < secs and not (stop_event is not None and stop_event.is_set()):
            ...
            if recov is None and now - last_col_t > 4.0:
                if len(fhist) >= 8 and now - fhist[0][0] >= 0.9 and mvd < 0.05:   # BAJADO 1.5->0.9s: para antes
                    contact = True; ctype = "odom"
                elif (IMU_CONTACT and prev_fwd and cur_lt is not None and len(fhist) >= 5
                    ...
                    if cur_lt > bl * 1.5 + 3.0 or cur_ah > ba + 1.8:   # mas sensible (par/accel)
            ...
            # --- RECUPERACION: MINI paso atras (corto, con chequeo trasero) para des-encajarse + pivota ---
            if recov is not None:
                el = now - recov["t0"]
                if recov["ph"] == "BACK":                 # mini retroceso BREVE y LENTO (solo si hay hueco detras)
                    rear = clear_ahead(cdp, 180)
                    if el < 0.45 and rear > 0.6:          # ~0.45s a velocidad reducida -> se separa del obstaculo
                        cmd = (0, -0.35, 0, 0); ph = "R-BACK"   # mini, por ENCIMA de la deadzone (0.3)
                    else:
                        recov = {"ph": "TURN", "t0": now}; el = 0
                if recov is not None and recov["ph"] == "TURN":
                    if el < 1.3:
            ...
            if brk is not None:                           # encajonado/mucho peligro: MINI atras + giro grande
                el = now - brk["t0"]
                if brk["ph"] == "BACK":                   # mini retroceso BREVE (con chequeo trasero) para crear hueco
                    rear = clear_ahead(cdp, 180)
                    if el < 0.45 and rear > 0.6:
                        cmd = (0, -0.35, 0, 0); ph = "BRK-BK"   # mini atras, por encima de la deadzone
                    else:
            ...
            if cmd is not None:
                ...
                if True:
                    ...
                    # CAMARA-BAJA: la mesa/pared blanca que el LiDAR de cabeza NO ve (vstrong) -> a cobs (capa
                    # decayente) para que el A* la rodee y el DWA la esquive. (Solo informa el mapa, no gira.)
                    if vb and vstrong and tgt is not None:
                        h0 = math.radians(yaw)
                        for dd in (0.5, 0.7):
                            oc = (round((x + dd * math.cos(h0)) / OCELL), round((y + dd * math.sin(h0)) / OCELL))
                            cobs[oc] = now + COBS_TTL
                    ...
                    if tgt is not None and ((not plan_pts) or (tgt != tgt_planned) or (now - plan_t > PLAN_SEC)):
                        ...
                        if not plan_pts:                              # A* no llega -> descarta esa frontera 12s y prueba otra
                            ...
                            tgt = None; tgt_t = now
                    # --- CONTROL LOCAL = DWA (logica del firmware): sigue el carrot del path A* puntuando
                    #     trayectorias por progreso + holgura. Suave, evita obstaculos directamente, NO se enrosca. ---
                    if tgt is not None and plan_pts:
                        carrot = path_carrot(plan_pts, x, y)
                        op = [(cx * OCELL, cy * OCELL) for (cx, cy) in oset
                              if abs(cx * OCELL - x) < 2.6 and abs(cy * OCELL - y) < 2.6]   # obstaculos cercanos
                        _, lyc, rxc, _, lbl = dwa_step(x, y, yaw, carrot, op)
                        cmd = (0, lyc, rxc, 0); ph = lbl
                        if lyc == 0 and rxc == 0:                     # DWA no halla motriz segura -> encajonado
                            nstop += 1
                            if nstop > 12:                            # ~1.2s parado (mucho peligro) -> mini atras + giro
                                cl = clear_ahead(cdp, +55); cr = clear_ahead(cdp, -55)
                                brk = {"ph": "BACK", "t0": now, "dir": -AV_TURN if cl >= cr else AV_TURN}
                                tgt = None; nstop = 0; lg.write("BOXED(dwa) -> mini atras + giro de salida\n")
                        else:
                            nstop = 0
                    else:                                # sin ruta/objetivo -> avanza recto (replanifica)
