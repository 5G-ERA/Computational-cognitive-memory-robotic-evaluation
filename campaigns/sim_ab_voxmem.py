#!/usr/bin/env python3
"""Campana A/B de la MEMORIA DE VOXELS (Renxi 14-ago, spatio_temporal_voxel_layer).

Ambos brazos llevan la pila META completa y ruido calibrado: lo UNICO que cambia es
G1_VOXMEM. La hipotesis es que retener las celdas confirmadas mientras caen en la banda
ciega del laser (<1.2 m) evita las colisiones "con el laser diciendo libre".
Preparacion offline (244 nubes reales de colision): con TTL 3 s se sostienen ~29 celdas
por instante y NINGUNA invade la holgura del DWA sobre la trayectoria posterior real;
con TTL 6 s ya lo hacen en el 3% de los casos. De ahi el TTL por defecto.
Orden ABBA intercalado. Progreso por stdout."""
...
COMMON = {"G1_META2": "2", "G1_M2_CFG": "config_meta2_g1payload.json",
          "G1_SIM_ID": "lab_v1_vox", "G1_SIM_NOISE": "1", "G1_M2_L3": "1", "G1_M2_L4": "1",
          "G1_M2_PROFILES": "1", "G1_M2_DOORLIB": "1", "G1_M2_FRAGSPEED": "1",
          "G1_M2_ABORT_WIN": "150", "G1_M2_ABORT_PROG": "0.25", "G1_M2_HELP_S": "16"}
# META identico en los dos brazos: se aisla la memoria, nada mas.
_META = {"G1_METASM": "1", "G1_RETREAT": "1"}
ARMS = {
    "vxoff": dict(_META, **{"G1_VOXMEM": "0", "G1_M2_STATE": "meta2_state_vox_off.json"}),
    "vxon":  dict(_META, **{"G1_VOXMEM": "1", "G1_VOXMEM_TTL": "3.0", "G1_VOXMEM_R": "1.2",
                            "G1_M2_STATE": "meta2_state_vox_on.json"}),
}
ORDER = ["vxoff", "vxon", "vxon", "vxoff", "vxoff", "vxon", "vxon", "vxoff",
         "vxoff", "vxon", "vxon", "vxoff"]
...
def analyze(path, arm):
    ...
    return {"arm": arm, "file": os.path.basename(path),
            ...
            "vox_inj_med": (lambda v: (sorted(v)[len(v) // 2] if v else 0))(
                [x["vox_inj"] for x in ss if isinstance(x.get("vox_inj"), (int, float))])}
...
for i, arm in enumerate(ORDER):
    ...
    logpath = os.path.join(SCRATCH, "vox_%02d_%s.log" % (i, arm))
    before = newest_run()
...
json.dump(results, open(os.path.join(SCRATCH, "vox_results.json"), "w"), indent=1)
