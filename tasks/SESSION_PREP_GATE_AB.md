...
Measured on the lab iPad (27-Aug): the video is **1280x720 native**, ~240 KB per photo, so about **850 MB/hour at 1 Hz** (the earlier 250 MB/h estimate assumed a smaller frame). GPUEDGE has over 1 TB free, so even a long session fits; use `--hz 0.5` if a very long pass is planned.
